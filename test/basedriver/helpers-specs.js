import { isPackageOrBundle, parseCapabilities, validateCapabilities, mergeCapabilities, matchCapabilities, processCapabilities } from '../../lib/basedriver/helpers';
...
describe('helpers', () => {
  ...
  describe('#validateCapabilities', () => {
    it('returns invalid argument error if "capability" is not a JSON object (1)', () => {
      for (let arg of [undefined, null, 1, true, 'string']) { 
        (function () { validateCapabilities(arg); }).should.throw(/must be a JSON object/);
      }
    });
  ...
  describe('#parseCapabilities', () => {
    ...
    it('returns invalid argument error if "capabilities" is not an object (1.1)', () => {
      for (let arg of [undefined, null, 1, true, 'string']) {
        (function (){ parseCapabilities(arg); }).should.throw(/must be a JSON object/);
      }
    });
    ...
    it('returns invalid argument error if "firstMatch" is not an array and is not undefined (3.2)', () => {
      for (let arg of [null, 1, true, 'string']) {
        (function (){ parseCapabilities({}, {}, arg); }).should.throw(/must be a JSON array or undefined/);
      }
    });
