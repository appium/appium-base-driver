...
class ProxyRequestError extends ES6Error {
  constructor (err, response, statusCode) {
    let message = `Proxy request unsuccessful. ${util.hasValue(response) ? response.value : ''}`;
    super(err || message);
    this.w3cStatus = HTTPStatusCodes.BAD_REQUEST;
  ...
  getActualError () {
    if (util.hasValue(this.response) && util.hasValue(this.response.status) && util.hasValue(this.response.value)) {
      //returns actual error cause for request failure based on `jsonwp.status`
      return errorFromCode(this.response.status, this.response.value);
    } else if (util.hasValue(this.response) && util.hasValue(this.response.value) && util.hasValue(this.response.value.error)) {
      const error = new MJSONWPError(
        this.response.value.message,
        null,
        this.statusCode,
        this.response.value.error,
      );
      if (this.response.value.stacktrace) {
        error.stack = this.response.value.stacktrace;
      }
      return error;
    }
...
function isErrorType (err, type) {
  ...
  if (type.name === MJSONWPError.name) {
    ...
    if (response) {
      let hasJsonwpObj = response.status;
      let hasW3CObj = _.isPlainObject(err.response.value);
      if (hasJsonwpObj || hasW3CObj) {
        return true;
      }
    }
    return false;
