...
class ProxyRequestError extends ES6Error {
  constructor (err, jsonwp, statusCode) {
  ...
  getActualError () {
    ...
    } else if (util.hasValue(this.jsonwp) && util.hasValue(this.jsonwp.value) && util.hasValue(this.jsonwp.value.error)) {
      const error = new MJSONWPError(
        this.jsonwp.value.message,
        null,
        this.statusCode,
        this.jsonwp.value.error,
      );
      if (this.jsonwp.value.stacktrace) {
        error.stack = this.jsonwp.value.stacktrace;
      }
      return error;
    }
...
function isErrorType (err, type) {
  ...
  if (type.name === MJSONWPError.name) {
    ...
    if (hasJsonwpObj) {
      return !!err.jsonwp.status || _.isPlainObject(err.jsonwp.value);
    }
    return false;
  }
