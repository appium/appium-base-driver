...
function makeArgs (reqParams, jsonObj, payloadParams) {
  ...
  if (_.isFunction(payloadParams.makeArgs)) {
    // if the spec defines a 'makeArgs' function, allow it to figure out how to
    // turn the json inputs into a command arguments array
    ...
    args = _.flatten(realRequiredParams).map((p) => jsonObj[p]);
    if (payloadParams.optional) {
      args = args.concat(_.flatten(payloadParams.optional).map((p) => jsonObj[p]));
    }
  }
  // finally, get our url params (session id, element id, etc...) on the end
