...
class JWProxy {
  ...
  async getDownstreamProtocol () {
    const [, resBody] = await this.proxy(`${this.scheme}://${this.server}:${this.port}${this.base}/status`, 'GET');
    return this.getProtocolFromResBody(resBody);
  }
  ...
  async proxyCommand (url, method, body = null) {
    ...
    if (method.toLowerCase() === 'post' && url.match(/\/timeouts$/))  {
      return await this.proxySetTimeouts(url, method, body);
    }

    // Same arguments, but different URLs

    const proxyToFixedEndpoint = async (urlPatterns, expectedMethod, jsonwpUrlReplacer, w3cUrlReplacer) => {
      if (method.toLowerCase() !== expectedMethod || !_.some(urlPatterns, (x) => url.match(x))) {
        return;
      }

      let downstreamProtocol;
      try {
        log.info(`The '${url}' API might not be the same for the downstream protocol. ` +
          'Trying to proxy as is');
        return await this.proxy(url, method, body);
      } catch (e) {
        try {
          downstreamProtocol = await this.getDownstreamProtocol();
        } catch (err) {
          log.warn(`The downstream protocol cannot be detected. ` +
      ...
      if (downstreamProtocol === MJSONWP) {
        const rewrittenUrl = jsonwpUrlReplacer(url);
        log.info(`Rewriting the URL to ${rewrittenUrl}`);
        return await this.proxy(rewrittenUrl, method, body);
      }
      if (downstreamProtocol === W3C) {
        const rewrittenUrl = w3cUrlReplacer(url);
        log.info(`Rewriting the URL to ${rewrittenUrl}`);
        return await this.proxy(rewrittenUrl, method, body);
      }
    };

    const executeMethodPatterns = [/\/execute$/, /\/execute[\/_]a?sync$/];
    const executeJsonwpReplacer = (url) => url.replace(/\/execute.*/,
      url.includes('async') ? '/execute_async' : '/execute');
    const executeW3cReplacer = (url) => url.replace(/\/execute.*/,
      url.includes('async') ? '/execute/async' : '/execute/sync');

    const elementScreenshotMethodPatterns = [/\/element\/[^\/]+\/screenshot$/, /\/screenshot\/.+$/];
    const elementScreenshotJsonwpReplacer = (url) => url.replace(/\/element\/([^\/]+)\/screenshot$/,
      '/screenshot/$1');
    const elementScreenshotW3cReplacer = (url) => url.replace(/\/screenshot\/(.+)$/,
      '/element/$1/screenshot');

    for (const args of [
      [executeMethodPatterns, 'post', executeJsonwpReplacer, executeW3cReplacer],
      [elementScreenshotMethodPatterns, 'get', elementScreenshotJsonwpReplacer, elementScreenshotW3cReplacer],
    ]) {
      const result = await proxyToFixedEndpoint(...args);
      if (result) {
        return result;
      }
    }
