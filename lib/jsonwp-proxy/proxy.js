...
function truncateBody (content) {
  // slice is needed as a workaround for string interning op
  return ` ${_.truncate(
    _.isString(content) ? content : JSON.stringify(content),
    { length: LOG_OBJ_LENGTH })}`.slice(1);
}
...
class JWProxy {
  ...
  async proxy (url, method, body = null) {
    ...
    try {
      ...
      let resBodyObj = res.body;
      if (!_.isPlainObject(resBodyObj)) {
        try {
          resBodyObj = JSON.parse(resBodyObj);
        } catch (ign) {}
      }
      if (!_.isPlainObject(resBodyObj)) {
  ...
  async command (url, method, body = null) {
    ...
    throw new errors.UnknownError(`Did not know what to do with response code '${response.statusCode}' ` +
                                  `and response body '${truncateBody(response.body)}'`);
  }
