...
class JWProxy {
  ...
  async proxyCommand (url, method, body = null) {
    ...
    const proxyToFixedEndpoint = async (commandNames, jsonwpUrlFixer, w3cUrlFixer) => {
      if (!_.isArray(commandNames)) {
      ...
      try {
        ...
        switch (downstreamProtocol) {
          case MJSONWP:
            rewrittenUrl = jsonwpUrlFixer();
            break;
          case W3C:
            rewrittenUrl = w3cUrlFixer();
            break;
          default:
    ...
    const executeJsonwpFixer = () => url.replace(/\/execute.*/,
      url.includes('async') ? '/execute_async' : '/execute');
    const executeW3cFixer = () => url.replace(/\/execute.*/,
      url.includes('async') ? '/execute/async' : '/execute/sync');

    const elementScreenshotCommand = 'getElementScreenshot';
    const elementScreenshotJsonwpFixer = () => url.replace(/\/element\/([^\/]+)\/screenshot$/,
      '/screenshot/$1');
    const elementScreenshotW3cFixer = () => url.replace(/\/screenshot\/(.+)$/,
      '/element/$1/screenshot');

    for (const args of [
      [executeCommands, executeJsonwpFixer, executeW3cFixer],
      [elementScreenshotCommand, elementScreenshotJsonwpFixer, elementScreenshotW3cFixer],
    ]) {
