...
class JWProxy {
  ...
  async proxy (url, method, body = null) {
    ...
    try {
      ...
      if (res.statusCode < 400 && _.isPlainObject(resBodyObj) && util.hasValue(resBodyObj.status) && resBodyObj.status !== 0) {
  ...
  async proxyReqRes (req, res) {
    ...
    let body;
    try {
      [response, body] = await this.proxy(req.originalUrl, req.method, req.body);
    } catch (err) {
      if (isErrorType(err, errors.ProxyRequestError)) {
        throw err.getActualError();
      }
