...
class JWProxy {
  ...
  async getDownstreamProtocol () {
    const [, resBody] = await this.proxy(`${this.scheme}://${this.server}:${this.port}/wd/hub/status`, 'GET');
    return this.getProtocolFromResBody(resBody);
  }

  async proxyCommand (url, method, body = null) {
    const pathMatch = /(\/wd\/hub\/.+)/.exec(url);
    const commandName = pathMatch ? routeToCommandName(pathMatch[1], method.toUpperCase()) : null;
    if (!commandName) {
    ...
    const proxyToFixedEndpoint = async (commandNames, jsonwpUrlReplacer, w3cUrlReplacer) => {
      ...
      try {
        ...
        switch (downstreamProtocol) {
          case MJSONWP:
            rewrittenUrl = jsonwpUrlReplacer(url);
            break;
          case W3C:
            rewrittenUrl = w3cUrlReplacer(url);
            break;
          default:
        ...
        if (rewrittenUrl === url) {
          log.info(`Did not know how to rewrite the original URL '${url}'`);
          throw e;
        }
    ...
    const executeCommands = ['execute', 'executeAsync'];
    ...
    const elementScreenshotCommands = ['getElementScreenshot'];
    ...
    for (const args of [
      [executeCommands, executeJsonwpReplacer, executeW3cReplacer],
      [elementScreenshotCommands, elementScreenshotJsonwpReplacer, elementScreenshotW3cReplacer],
    ]) {
