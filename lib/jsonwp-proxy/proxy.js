    ...
    const conflicts = [
      {
        ...
        jsonwpConverter: () => url.replace(/\/execute.*/,
          url.includes('async') ? '/execute_async' : '/execute'),
        w3cConverter: () => url.replace(/\/execute.*/,
          url.includes('async') ? '/execute/async' : '/execute/sync'),
      },
      {
        commandNames: ['getElementScreenshot'],
        jsonwpConverter: () => url.replace(/\/element\/([^\/]+)\/screenshot$/,
          '/screenshot/$1'),
        w3cConverter: () => url.replace(/\/screenshot\/(.+)$/,
          '/element/$1/screenshot'),
      },
    ];

    for (const {commandNames, jsonwpConverter, w3cConverter} of conflicts) {
      ...
      try {
        ...
        switch (downstreamProtocol) {
          case MJSONWP:
            rewrittenUrl = jsonwpConverter();
            break;
          case W3C:
            rewrittenUrl = w3cConverter();
            break;
          default:
