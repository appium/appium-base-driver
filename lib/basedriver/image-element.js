...
class ImageElement {
  ...
  static async execute (driver, cmd, imgElId) {
    ...
    switch (cmd) {
      case AVAILABLE_COMMANDS.click:
        return await imgEl.click(driver);
      case AVAILABLE_COMMANDS.elementDisplayed:
        return true;
      case AVAILABLE_COMMANDS.getSize:
        return imgEl.size;
      case AVAILABLE_COMMANDS.getLocation:
      case AVAILABLE_COMMANDS.getLocationInView:
        return imgEl.location;
      case AVAILABLE_COMMANDS.getElementRect:
        return imgEl.rect;
...
function isImageCommand (args, cmd) {
  return !!(getImgElFromArgs(args) && AVAILABLE_COMMANDS[cmd]);
}

export { ImageElement, getImgElFromArgs, makeImageElementCache, isImageCommand,
