...
class ImageElement {
  ...
  async click (driver) {
    ...
    if (imageElementTapStrategy === IMAGE_EL_TAP_STRATEGY_W3C) {
      ...
      try {
        return await this.doActualTap(driver, command, method, url, action);
      } catch (e) {
        // If we got an error about performActions not being implemented,
        // ignore it and fall back to the other strategy
        if (!e.message.includes('Driver does not seem to implement')) {
          throw e;
        }
      }
    ...
    const method = 'POST';
    const url = '/touch/perform';
    const command = 'performTouch';
    const action = {
      action: 'tap',
      options: {x, y}
    };
  ...
  /**
   * Use command or route information to perform an action on a driver
   *
   * @param {BaseDriver} driver - the driver to use for the tap
   * @param {string} command - the command name to use with the driver
   * @param {string} method - the HTTP method for proxying the command
   * @param {string} url - the url for proxying the command
   * @param {object} action - the touch action payload
   *
   * @returns {object} - result of performing the tap
   */
  async doActualTap (driver, command, method, url, action) {
    // if we're proxying to a downstream driver, we are no longer in an
    // execution path that will do the proxy automatically for us. Instead,
    // construct the call to the action endpoint and manually proxy (that is,
    // if we would have proxied this route normally)
    if (driver.proxyActive() && !driver.proxyRouteIsAvoided(null, method, url)) {
      return await driver.proxyCommand(url, method, {actions: [action]});
    }

    // if we weren't proxying, also check to see whether the command is
    // actually implemented on the driver
    if (driver[command]) {
      return await driver[command]([action]);
    }

    throw new Error(`Driver does not seem to implement '${command}' command`);
  }
...
export { ImageElement, getImgElFromArgs, makeImageElementCache,
