...
commands.timeouts = async function (timeoutsObj) {
  if (_.has(timeoutsObj, 'type') && _.has(timeoutsObj, 'ms')) {
    const {type, ms} = timeoutsObj;
    log.debug(`MJSONWP timeout argument: ${JSON.stringify(timeoutsObj)}}`);
  ...
  log.debug(`W3C timeout argument: ${JSON.stringify(timeoutsObj)}}`);
