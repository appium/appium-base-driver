...
commands.createSession = async function (desiredCapabilities, requiredCaps, capabilities) {
  ...
  if (!desiredCapabilities && !capabilities) {
    throw new Error('Parameters were incorrect. You need to provide "desiredCapabilities" or "capabilities" JSON.');
    ...
    if (!caps) {
      throw new Error(`No matching capabilities found for: ${JSON.stringify(capabilities)}`);
    }
  }
