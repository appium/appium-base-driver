...
commands.createSession = async function (desiredCapabilities, requiredCaps, capabilities) {
  if (this.sessionId !== null) {
    this.log.errorAndThrow(new errors.SessionNotCreatedError('Cannot create a new session ' +
  ...
  if (this.opts.noReset && this.opts.fullReset) {
    this.log.errorAndThrow(new Error("The 'noReset' and 'fullReset' capabilities are mutually " +
  ...
  // We need to ininitialize one onUnexpectedShutdown promise per session
  // to avoid the promise fullfilment being propagated between sessions.
...
function fixCaps (originalCaps, desiredCapConstraints = {}, logger = {}) {
  ...
  for (let cap of booleanCaps) {
    ...
    if (_.isString(value)) {
      ...
      if (value === 'true' || value === 'false') {
        logger.warn(`Capability '${cap}' changed from string to boolean. This may cause unexpected behavior`);
        caps[cap] = (value === 'true');
      }
    }
  }
  ...
  for (let cap of intCaps) {
    ...
    if (_.isString(value)) {
      ...
      logger.warn(`Capability '${cap}' changed from string ('${value}') to integer (${newValue}). This may cause unexpected behavior`);
      caps[cap] = newValue;
    }
  }
