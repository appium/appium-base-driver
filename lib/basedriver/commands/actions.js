...
commands.performActions = async function () {
...
helpers.translateActions = function (actionsArr) {
  ...
  for (let touchAction of actionsArr) {
    ...
    if (!ACTION_TYPES[type]) {
      ...
      throw new Error(`${type} actions are not supported by this driver`);
    }
    ...
    if (parameters && parameters.pointerType) {
      ...
      if (!POINTER_TYPES[pointerType]) {
        throw new Error(`${parameters.pointerType} is not a valid pointerType`);
      } else if (pointerType !== POINTER_TYPES.TOUCH) {
        throw new Error(`${parameters.pointerType} is not supported by this driver`);
      }
    }
