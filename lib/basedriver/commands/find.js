...
import { imageUtils } from 'appium-support';
 ...
 * @property {boolean} multiple - Whether we are finding one element or
 * multiple
 */
...
helpers.findByImage = async function (b64Template, {
  ...
  if (this.settings.getSettings().fixImageTemplateSize) {
  ...
  try {
    ...
    // error if an element is not found eventually. In this case, we will
...
helpers.ensureTemplateSize = async function (b64Template, screenWidth, screenHeight) {
  let imgObj = await imageUtils.getJimpImage(b64Template);
  ...
  return (await imgObj.getBuffer(imageUtils.MIME_PNG)).toString('base64');
};
...
helpers.getScreenshotForImageFind = async function (screenWidth, screenHeight) {
  ...
  let imgObj = await imageUtils.getJimpImage(b64Screenshot);
  ...
  if (screenWidth !== shotWidth) {
    log.info(`Scaling screenshot width from ${shotWidth}px to match screen ` +
             `width of ${screenWidth}px`);
