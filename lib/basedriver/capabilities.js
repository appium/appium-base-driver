...
function parseCaps (caps, constraints = {}, shouldValidateCaps = true) {
  ...
  let filterredConstraints = {...constraints};
  let requiredCapsKeys = _.keys(requiredCaps);
  for (let key of _.keys(filterredConstraints)) {
    if (requiredCapsKeys.includes(key)) {
      delete filterredConstraints[key];
    }
  }
  ...
  let validatedFirstMatchCaps = allFirstMatchCaps.map((firstMatchCaps) => {
    try {
      ...
      return shouldValidateCaps ? validateCaps(firstMatchCaps, filterredConstraints) : firstMatchCaps;
    } catch (e) {
