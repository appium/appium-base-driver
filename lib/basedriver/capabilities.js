...
function validateCaps (caps, constraints = {}, opts = {}) {
  ...
  if (skipPresenceConstraint) {
    ...
    // If the key is present in the 'otherKeys' array strip out the presence
    // constraint because the constraint has already passed
    /*for (let key of _.keys(constraints)) {
...
function parseCaps (caps, constraints = {}, shouldValidateCaps = true) {
  ...
  // Remove the 'presence' constraint if the for keys that are already present in 'requiredCaps'
  let constraintsCopy = _.clone(constraints);
  let requiredCapsKeys = _.keys(requiredCaps);
  for (let key of _.keys(constraintsCopy)) {
  ...
  let validatedFirstMatchCaps = allFirstMatchCaps.map((firstMatchCaps) => {
    try {
      ...
      return shouldValidateCaps ? validateCaps(firstMatchCaps, constraintsCopy, {otherKeys: _.keys(requiredCaps)}) : firstMatchCaps;
    } catch (e) {
      return null;
    }
...
function processCapabilities (caps, constraints = {}, shouldValidateCaps = true) {
  const {matchedCaps} = parseCaps(caps, constraints, shouldValidateCaps);

  if (matchedCaps === null) {
    // TODO: Make the constraints verbose
    throw new Error(`Could not find matching capabilities from ${caps}`);
  }
