...
function mergeCaps (primary = {}, secondary = {}) {
  ...
  for (let [name, value] of _.toPairs(secondary)) {
    // Overwriting is not allowed. Primary and secondary must have different properties (w3 rule 4.4)
...
function parseCaps (caps, constraints = {}, shouldValidateCaps = true) {
  ...
  // Validate the firstMatch capabilities one-by one (see spec #5)
  ...
  // Try to match requiredCaps with one of the validated firstMatch capabilities (see spec #6)
