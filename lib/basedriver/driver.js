...
class BaseDriver extends MobileJsonWireProtocol {
  ...
  initLogger () {
    ...
    this.log = logger.getLogger(
      () => `${instanceName}@${instanceId}` +
            ((_.isString(this.sessionId) && this.sessionId.length >= 8) ? ` [${this.sessionId.substring(0, 8)}]` : '')
    );
  }
  ...
  logEvent (eventName) {
    if (eventName === "commands") {
      this.log.errorAndThrow(new Error("Cannot log commands directly"));
    }
    if (typeof eventName !== "string") {
      this.log.errorAndThrow(new Error(`Invalid eventName ${eventName}`));
    }
  ...
  async executeCommand (cmd, ...args) {
    ...
    if (!this[cmd]) {
      this.log.errorAndThrow(new errors.NotYetImplementedError());
    }
    ...
    if (this.isCommandsQueueEnabled) {
      ...
      if (this.shutdownUnexpectedly) {
        this.log.errorAndThrow(new errors.NoSuchDriverError('The driver was unexpectedly shut down!'));
      }
  ...
  validateLocatorStrategy (strategy, webContext = false) {
    ...
    if (!_.includes(validStrategies, strategy)) {
      this.log.errorAndThrow(new errors.InvalidSelectorError(`Locator Strategy '${strategy}' is not supported for this session`));
    }
  }
