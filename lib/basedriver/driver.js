...
class BaseDriver extends Protocol {
  ...
  async executeCommand (cmd, ...args) {
    ...
    const commandExecutor = async () => imgElId
      ...
      : await this[cmd](...args);
    const res = await this.commandsQueueGuard.acquire(BaseDriver.name, async () => await commandExecutor(), {
      skipQueue: !this.isCommandsQueueEnabled || cmd === 'executeDriverScript',
    });
