...
async function configureApp (app, supportedAppExtensions) {
  ...
  return await APPLICATIONS_CACHE_GUARD.acquire(app, async () => {
    if (isUrl) {
      ...
      if (ZIP_EXTS.includes(path.extname(newApp))) {
        fileName = sanitize(path.basename(newApp), {
      ...
      if (!fileName) {
        // assign the default extension if none has been detected
        fileName = `appium-app${_.first(supportedAppExtensions)}`;
      }
...
/**
 * Recursively find all instances of the key 'inKey' and makes a copy of them called 'outKey'
 * @param {*} input Any type of input
 * @param {String} firstKey The key name to replace
 * @param {String} secondKey The key name to replace it with
 */
function duplicateKeys (input, firstKey, secondKey) {
  if (_.isArray(input)) {
    ...
    for (let [key, value] of _.toPairs(input)) {
      if (key === firstKey) {
        resultObj[secondKey] = duplicateKeys(value, firstKey, secondKey);
      }
      if (key === secondKey) {
        resultObj[firstKey] = duplicateKeys(value, firstKey, secondKey);
      }
      resultObj[key] = duplicateKeys(value, firstKey, secondKey);
    }
    return resultObj;
  }
