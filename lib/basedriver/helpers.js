...
async function unzipApp (zipPath, dstRoot, supportedAppExtensions) {
  ...
  try {
    ...
    await fs.walkDir(tmpRoot, true, (itemPath) => allExtractedItems.push(itemPath) && false);
    ...
    let allBundleItems = [];
    await fs.walkDir(tmpRoot, true, (itemPath) => allBundleItems.push(itemPath) && false);
    allBundleItems = allBundleItems
      .map((itemPath) => path.relative(tmpRoot, itemPath))
      .filter((relativePath) => isSupportedAppItem(relativePath))
