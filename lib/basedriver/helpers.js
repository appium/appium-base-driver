...
async function configureApp (app, supportedAppExtensions, mountRoot = "Volumes", windowsShareUserName = "", windowsSharePassword = "") {
  ...
  if (newApp.startsWith('\\\\')) {
    ...
    if (headers['last-modified']) {
      logger.debug(`Last-Modified header: ${headers['last-modified']}`);
    ...
    if (headers['content-type']) {
      logger.debug(`'Content-Type' header: ${headers['content-type']}`);
    ...
    if (headers['content-disposition'] && /^attachment/i.test(headers['content-disposition'])) {
      ...
      if (match) {
        ...
        fileName = match[1];
        shouldUnzipApp = _.includes(ZIP_EXTS, path.extname(fileName));
      }
    }
