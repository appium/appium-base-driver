 ...
 */
function copyKey (input, inKey, outKey) {
  if (_.isArray(input)) {
    return input.map((item) => copyKey(item, inKey, outKey));
  } else if (_.isPlainObject(input)) {
    const resultObj = {};
    for (let [key, value] of _.toPairs(input)) {
      if (key === inKey) {
        resultObj[outKey] = copyKey(value, inKey, outKey);
      }
      resultObj[key] = copyKey(value, inKey, outKey);
    }
    return resultObj;
  }
...
export {
  configureApp, isPackageOrBundle, getCoordDefault, getSwipeTouchDuration, copyKey,
};
