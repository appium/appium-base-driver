...
const PROTOCOLS_MAPPING_CACHE = new LRU({
  max: 100,
});

function cacheProtocolValue (value, sessionId = null) {
  if (!sessionId) {
    return value;
  }
  if (PROTOCOLS_MAPPING_CACHE.has(sessionId)) {
    return PROTOCOLS_MAPPING_CACHE.get(sessionId);
  }
  if (!value) {
    return null;
  }
  ...
  let asyncHandler = async (req, res) => {
    ...
    try {
      ...
      if (spec.command === CREATE_SESSION_COMMAND) {
        ...
        currentProtocol = BaseDriver.determineProtocol(...(makeArgs(
          req.params, jsonObj, spec.payloadParams || {}, currentProtocol)));
      }
      ...
      if (parsedDriverRes) {
        currentProtocol = parsedDriverRes.protocol || currentProtocol;
        if (spec.command === CREATE_SESSION_COMMAND && !_.isEmpty(parsedDriverRes.value)) {
          cacheProtocolValue(currentProtocol, parsedDriverRes.value[0]);
        }
      ...
      currentProtocol = extractProtocol(driver, req.params.sessionId || newSessionId) || currentProtocol;
