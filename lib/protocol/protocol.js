...
function buildHandler (app, method, path, spec, driver, isSessCmd) {
  let asyncHandler = async (req, res) => {
    ...
    try {
      ...
      let protocolLogger = getLogByProtocol(currentProtocol);
      ...
      protocolLogger.debug(`Calling ${driver.constructor.name}.${spec.command}() with args: ` +
      ...
      protocolLogger = getLogByProtocol(currentProtocol);

      // unpack createSession response
      if (spec.command === CREATE_SESSION_COMMAND) {
        newSessionId = driverRes[0];
        cacheProtocolValue(currentProtocol, newSessionId);
        protocolLogger.debug(`Cached the protocol value '${currentProtocol}' for the new session ${newSessionId}`);
