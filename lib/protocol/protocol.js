...
function buildHandler (app, method, path, spec, driver, isSessCmd) {
  let asyncHandler = async (req, res) => {
    ...
    try {
      ...
      getLogByProtocol(currentProtocol).debug(`Calling ${driver.constructor.name}.${spec.command}() with args: ` +
      ...
      if (spec.command === CREATE_SESSION_COMMAND) {
        ...
        getLogByProtocol(currentProtocol)
      ...
      getLogByProtocol(currentProtocol).debug(`Responding to client with driver.${spec.command}() ` +
