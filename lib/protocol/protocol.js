...
const genericProtocolLog = logger.getLogger('GENERIC');
...
// but the client still tries to send a command to the same session id.
// So we know how to properly wrap the error message for it
const PROTOCOLS_MAPPING_CACHE = {};
const MAX_PROTOCOLS_MAPPING_CACHE_SIZE = 100;

function extractProtocol (driver, sessionId = null) {
  const cacheProtocolValue = (value) => {
    if (!sessionId) {
      return value;
    }
    if (PROTOCOLS_MAPPING_CACHE[sessionId]) {
      return PROTOCOLS_MAPPING_CACHE[sessionId];
    }
    if (!value) {
      return null;
    }

    PROTOCOLS_MAPPING_CACHE[sessionId] = [new Date().getTime(), value];
    if (_.keys(PROTOCOLS_MAPPING_CACHE).length > MAX_PROTOCOLS_MAPPING_CACHE_SIZE) {
      const oldestSessionId = _.toPairs(PROTOCOLS_MAPPING_CACHE)
        .reduce((a, b) => a[1][0] < b[1][0] ? a[0] : b[0]);
      delete PROTOCOLS_MAPPING_CACHE[oldestSessionId];
    }
    return value;
  };

  if (_.isFunction(driver.driverForSession)) {
    // Extract the protocol for the current session if the given driver is the umbrella one
    if (!sessionId) {
      return null;
    }

    const dstDriver = driver.driverForSession(sessionId);
    if (dstDriver) {
      return cacheProtocolValue(dstDriver.protocol);
    }
    const result = PROTOCOLS_MAPPING_CACHE[sessionId];
    if (!result) {
      return null;
    }
    return result[1];
  ...
  if (sessionId) {
    return cacheProtocolValue(driver.protocol);
  }
...
function buildHandler (app, method, path, spec, driver, isSessCmd) {
  let asyncHandler = async (req, res) => {
    ...
    const log = getLogByProtocol(currentProtocol);
    ...
    try {
      ...
      // TODO: Remove the executeCommand wrapping from the umbrella driver
      ...
      const protocolLogger = getLogByProtocol(currentProtocol);

      // unpack createSession response
      if (spec.command === 'createSession') {
        newSessionId = driverRes[0];
        if (currentProtocol === MJSONWP) {
          driverRes = driverRes[1];
        } else if (currentProtocol === W3C) {
          driverRes = {
      ...
      } else if (currentProtocol === MJSONWP) {
...
async function doJwpProxy (driver, req, res) {
  const log = getLogByProtocol(extractProtocol(driver, req.params.sessionId));
