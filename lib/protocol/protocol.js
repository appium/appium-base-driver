  ...
  let asyncHandler = async (req, res) => {
    ...
    try {
      ...
      if (isErrorType(err, errors.ProxyRequestError)) {
        protocolLogger.error(`Encountered internal error running command:  ${JSON.stringify(err)} ${err.stack}`);
        actualErr = err.getActualError();
      } else if (!isW3C && (!(isErrorType(err, ProtocolError) || isErrorType(err, errors.BadParametersError)))) {
        protocolLogger.error(`Encountered internal error running command: ${err.stacktrace || err.stack}`);
...
function driverShouldDoJwpProxy (driver, req, command) {
  ...
  if (stringBody && stringBody.includes(IMAGE_ELEMENT_PREFIX)) {
    return false;
  }
...
export { Protocol, routeConfiguringFunction, isSessionCommand, MJSONWP_ELEMENT_KEY, W3C_ELEMENT_KEY, IMAGE_ELEMENT_PREFIX, parseProtocol };
