...
function buildHandler (app, method, path, spec, driver, isSessCmd) {
  let asyncHandler = async (req, res) => {
    ...
    const protocolLogger = (protocol) => getLogByProtocol(protocol);
    ...
    try {
      ...
      protocolLogger(currentProtocol).debug(`Calling ${driver.constructor.name}.${spec.command}() with args: ` +
      ...
      if (spec.command === CREATE_SESSION_COMMAND) {
        ...
        protocolLogger(currentProtocol)
      ...
      if (spec.command === DELETE_SESSION_COMMAND) {
        ...
        protocolLogger(currentProtocol)
          .debug('But deleting session, so not returning');
        driverRes = null;
      }
      ...
      protocolLogger(currentProtocol).debug(`Responding to client with driver.${spec.command}() ` +
      ...
      protocolLogger(currentProtocol).debug(`Encountered internal error running command: ${errMsg}`);
