...
function extractProtocol (driver, sessionId = null) {
  ...
  if (_.isFunction(driver.driverForSession)) {
    ...
    return dstDriver
      ? cacheProtocolValue(dstDriver.protocol)
...
function buildHandler (app, method, path, spec, driver, isSessCmd) {
  let asyncHandler = async (req, res) => {
    ...
    try {
      ...
      driverRes = _.has(driver, 'executeCommand')
        ? await driver.executeCommand(spec.command, ...args)
        : await driver.execute(spec.command, ...args);

      // Get the protocol after executeCommand (when command is `createSession`, protocol is assigned within
      // createSession function)
      currentProtocol = spec.command === 'createSession'
        ? driver.protocol
