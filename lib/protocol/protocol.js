...
function buildLogger (protocol, sessionId = null) {
  if (sessionId) {
    if (SESSIONS_CACHE.has(sessionId)) {
      const log = SESSIONS_CACHE.get(sessionId).logger;
      if (log) {
        return log;
      }
    }
...
const SESSIONS_CACHE = new LRU({
  max: 100,
});

function cacheSession (value, sessionId) {
  if (sessionId && value) {
    SESSIONS_CACHE.set(sessionId, {
      protocol: value,
      logger: buildLogger(value, sessionId),
    });
  }
  return value;
}

function resetCachedLogger (sessionId) {
  if (SESSIONS_CACHE.has(sessionId)) {
    SESSIONS_CACHE.get(sessionId).logger = null;
  }
}
...
function extractProtocol (driver, sessionId = null) {
  ...
  return dstDriver ? dstDriver.protocol : (SESSIONS_CACHE.get(sessionId) || {}).protocol;
}
...
function buildHandler (app, method, path, spec, driver, isSessCmd) {
  let asyncHandler = async (req, res) => {
    ...
    try {
      ...
      buildLogger(currentProtocol, req.params.sessionId).debug(`Calling ` +
      ...
      if (spec.command === CREATE_SESSION_COMMAND) {
        ...
        cacheSession(currentProtocol, newSessionId);
        buildLogger(currentProtocol, newSessionId)
      ...
      if (spec.command === DELETE_SESSION_COMMAND) {
        buildLogger(currentProtocol, req.params.sessionId)
          .debug(`Received response: ${_.truncate(JSON.stringify(driverRes), {length: LOG_OBJ_LENGTH})}`);
        buildLogger(currentProtocol, req.params.sessionId).debug('But deleting session, so not returning');
        driverRes = null;
      }
      ...
      buildLogger(currentProtocol, req.params.sessionId || newSessionId).debug(`Responding ` +
      ...
      if (spec.command === DELETE_SESSION_COMMAND) {
        ...
        resetCachedLogger(req.params.sessionId);
      }
    } catch (err) {
      ...
      buildLogger(currentProtocol, req.params.sessionId || newSessionId).debug(`Encountered ` +
...
async function doJwpProxy (driver, req, res) {
  buildLogger(extractProtocol(driver, req.params.sessionId), req.params.sessionId)
